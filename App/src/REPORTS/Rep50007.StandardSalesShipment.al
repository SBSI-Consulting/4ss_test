report 50007 "Standard Sales Shipment"
{
    ...
    dataset
    {
        dataitem(Header; "Sales Shipment Header")
        {
            ...
            column(DocumentNo_Lbl; ShipmentNo())
            {
            }
            ...
            column(TrackingNo_Lbl; FieldCaption("Tracking No."))
            {
            }
            column(NetWeight; "Net Weight")
            {
            }
            column(NetWeightLbl; FieldCaption("Net Weight"))
            {
            }*/
            column(ShipmentNo; "No.")
            {
            }
            column(ShipmentNoLbl; FieldCaption("No."))
            {
            }
            ...
            dataitem(Line; "Sales Shipment Line")
            {
                ...
                column(AmountExcludingVAT_Line; CalcAmountExclVAT())
                {
                    AutoFormatExpression = GetCurrencyCode();
                    AutoFormatType = 1;
                }
                column(AmountExcludingVAT_Line_Lbl; AmountExclVATCaption())
                {
                }
    ...
    var
        ...
        SalesShipmentLbl: Label 'Delivery Note';
        YourSalesShipmentLbl: Label 'Delivery Note';
        ...
        AmountExclVATLbl: Label 'Amount';
        TotalAmount: Decimal;
        SalesShipmentLbl_FR: Label 'Bon de livraison';
        ShipmentNoLbl_FR: Label 'N° livraison';
        ShptMethodDescLbl_FR: Label 'Méthode de livraison';

        Item: Record Item;
        TariffNo: code[20];

    local procedure ShipmentNo(): Text[250]
    begin
        if Cust."Language Code" = 'FRA' then
            exit(ShipmentNoLbl_FR)
        else
            exit(ShipmentNoLbl);
    end;
    ...
    begin
        AmountExclVAT := Line."Unit Price" * Line.Quantity;
        exit(AmountExclVAT);
    end;

    local procedure AmountExclVATCaption(): Text
    begin
        if AmountExclVATLbl <> '' then
            exit(AmountExclVATLbl);
    end;
    // SBSI
